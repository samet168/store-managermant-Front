...
const ShowOrder = () => {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      setError(false);
      try {
        const res = await api.get(`/cashier/orders/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });

        // ✅ Fix: API returns { data: [ {...} ] } — array ដូច្នេះត្រូវ [0]
  ...
  if (loading) {
    return (
      <div className="so-page">
        ...
        <div className="so-skeleton" style={{ width: 200, height: 28, marginBottom: 18 }} />
        <div className="so-grid">
          ...
          <div className="so-card so-card-full">
            <div className="so-card-header" style={{ background: '#f8fafc' }}>
              <div className="so-skeleton" style={{ width: 140, height: 14 }} />
            </div>
            <div className="so-card-body">
              {[1, 2, 3].map(j => (
                <div key={j} style={{ display: 'flex', gap: 12, padding: '13px 0', borderBottom: '1px solid #f1f5f9' }}>
                  <div className="so-skeleton" style={{ width: 30, height: 13 }} />
                  <div className="so-skeleton" style={{ flex: 2, height: 13 }} />
                  <div className="so-skeleton" style={{ flex: 1, height: 13 }} />
                  <div className="so-skeleton" style={{ flex: 1, height: 13 }} />
                  <div className="so-skeleton" style={{ flex: 1, height: 13 }} />
                </div>
  ...
  if (error || !order) {
    return (
      <div className="so-page">
        <div className="so-bc">
          <span className="so-bc-link" onClick={() => navigate('/dashboard/cashier/orders')}>Orders</span>
          <span className="so-bc-sep">/</span>
          <span className="so-bc-cur">Order #{id}</span>
        </div>
        <div className="so-card" style={{ padding: 40, textAlign: 'center' }}>
          <div style={{ fontSize: 48, marginBottom: 12 }}>📭</div>
          <div style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', marginBottom: 6 }}>Order not found</div>
          <div style={{ fontSize: 13.5, color: '#94a3b8' }}>
            Order #{id} could not be loaded. It may have been deleted or you don't have access.
          </div>
          <button className="so-btn-back" style={{ marginTop: 20 }} onClick={() => navigate('/dashboard/cashier/orders')}>
            ← Back to Orders
          </button>
        </div>
      </div>
  ...
  const total = order.details?.reduce((s, d) => s + d.quantity * Number(d.price), 0) || 0;
        ...
        <div className="so-card">
          ...
          <div className="so-card-body">
            ...
            <div className="so-info-row">
              ...
              <span className="so-info-value muted" style={{ whiteSpace: 'pre-line' }}>
                ...
                <tr key={d.id}>
                  ...
                  <td data-label="Subtotal">
                    <span className="so-subtotal">
                      ${(d.quantity * Number(d.price)).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </span>
                  </td>
                </tr>
              ))}
