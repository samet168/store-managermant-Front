...
const Orders = () => {
  ...
  const fetchOrders = async (page = 1, searchTerm = '') => {
    setLoading(true);
    try {
      const res = await api.get(`cashier/orders/list?page=${page}&search=${searchTerm}`, {
  ...
  const handleAdd    = ()      => navigate('/dashboard/cashier/orders/add');
  const handleShow   = (id)    => navigate(`/dashboard/cashier/orders/show/${id}`);
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this order?')) return;
    try {
      await api.delete(`/cashier/orders/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
