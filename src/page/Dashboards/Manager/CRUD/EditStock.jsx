...
const EditStock = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      setLoading(true);
      try {
        const res = await api.get(`/manager/stock-logs/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await api.post(`/manager/stock-logs/${id}`, {
      ...
      setTimeout(() => navigate("/dashboard/manager/stock"), 1200);
    } catch (err) {
      console.error(err);
      setError(
  ...
  return (
    <div className="es-page">
      ...
      {/* Breadcrumb */}
      <div className="es-bc">
        <span className="es-bc-link" onClick={() => navigate("/dashboard/manager/stock")}>
          Stock Logs
        </span>
