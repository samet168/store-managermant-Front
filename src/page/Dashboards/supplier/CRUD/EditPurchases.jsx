...
.ap-btn-remove {
  ...
  transition: background .12s; flex-shrink: 0; font-size: 16px;
}
    ...
    <div className="ap-layout">
      <div>
        <div className="ap-card" style={{ marginBottom: 20 }}>
          <div className="ap-card-head" style={{ background: '#fafafa' }}>
            <div className="ep-sk" style={{ width: 160, height: 14 }} />
          </div>
        ...
        <div className="ap-card">
          ...
          <div className="ap-card-body">
            {[1,2].map(k => (
              <div key={k} className="ap-prod-row" style={{ marginBottom: 10 }}>
                <div className="ep-sk" style={{ width: '100%', height: 38 }} />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="ap-card">
        <div className="ap-card-head" style={{ background: '#fafafa' }}>
          <div className="ep-sk" style={{ width: 120, height: 14 }} />
        </div>
        <div className="ap-card-body">
          {[1,2,3].map(k => (
            <div key={k} style={{ display:'flex', justifyContent:'space-between', padding:'10px 0', borderBottom:'1px solid #f1f5f9' }}>
              ...
              <div className="ep-sk" style={{ width: 100, height: 13 }} />
            </div>
          ))}
        </div>
      </div>
...
export default function EditPurchases() {
  ...
  /* ── Load suppliers, products & existing purchase ── */
  useEffect(() => {
    const token   = localStorage.getItem('token');
    const headers = { Authorization: `Bearer ${token}` };

    Promise.all([
      api.get('/supplier/suppliers', { headers }).catch(() => ({ data: [] })),
      api.get('/supplier/products',  { headers }).catch(() => ({ data: [] })),
      api.get(`/supplier/purchases/${id}`, { headers }),
    ]).then(([supRes, prodRes, purchRes]) => {
      ...
      // purchase data
      const p = purchRes.data?.data ?? purchRes.data;
      setForm({
  ...
  const handleItem = (itemId, field, value) =>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await api.post(`/supplier/purchases/${id}`, {
      ...
      if (typeof msg === 'object') {
        ...
        setError(msg || 'Failed to update purchase.');
      }
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="ap-page">
        ...
        <div className="ap-header">
          <div className="ep-sk" style={{ width: 220, height: 28, marginBottom: 8 }} />
          <div className="ep-sk" style={{ width: 280, height: 13 }} />
        </div>
        <SkeletonForm />
      </div>
    );
  ...
  return (
    <div className="ap-page">
      ...
      <div className="ap-bc">
        ...
        <span className="ap-bc-sep">/</span>
        <span className="ap-bc-cur">Edit Purchase #{id}</span>
      </div>

      {/* Header */}
      <div className="ap-header">
        <h1 className="ap-title">Edit Purchase</h1>
        <div className="ap-sub">Update the details of purchase order #{id}</div>
      </div>
            ...
            <div className="ap-card">
              ...
              <div className="ap-card-body">
                ...
                <div className="ap-summary-row">
                  <span className="ap-summary-label">Purchase #</span>
                  <span className="ap-summary-val" style={{ fontFamily:'monospace' }}>#{id}</span>
                </div>
                <div className="ap-summary-row">
