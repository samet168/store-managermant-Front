...
const Orders = () => {
  ...
  const fetchOrders = async (page = 1, searchTerm = '') => {
    setLoading(true);
    try {
      const res = await api.get(`/manager/orders/list?page=${page}&search=${searchTerm}`, {
  ...
  const handleAdd    = ()      => navigate('/dashboard/manager/orders/add');
  const handleShow   = (id)    => navigate(`/dashboard/manager/orders/show/${id}`);
