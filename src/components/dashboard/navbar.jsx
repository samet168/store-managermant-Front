  ...
  return (
    <nav className="dnb">
      ...
      <div className="dnb__inner">
        ...
        <button className="dnb__sidebar-toggle" onClick={onSidebarToggle} aria-label="Toggle sidebar">
          <span /><span /><span />
        </button>
        ...
        <ul className="dnb__nav">
          {NAV_LINKS.map(({ href, label, icon }) => (
            <li key={href}>
              <a className={`dnb__link${isActive(href) ? ' dnb__link--active' : ''}`} href={href}>
      ...
      {/* Overlay */}
      {mobileOpen && <div className="dnb__overlay" onClick={() => setMobileOpen(false)} />}

      {/* Mobile drawer */}
      <div className={`dnb__drawer${mobileOpen ? ' dnb__drawer--open' : ''}`}>
        <div className="dnb__drawer-header">
          <a className="dnb__logo" href="/dashboard">
            <div className="dnb__logo-icon">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/>
          ...
          <button className="dnb__drawer-close" onClick={() => setMobileOpen(false)}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
